...
export default class SlotScene extends Phaser.Scene {
  ...
  create() {
    // Reiniciamos estado cada vez que se lanza la escena
    this.ruletas = [];
    this.girando = false;
    ...
    // Botón tirar
    this.botonTirar = this.add.rectangle(GAME_WIDTH/2, GAME_HEIGHT/2 + 60, 280, 55, 0xFFD700)
      .setInteractive({ useHandCursor: true });

    this.add.text(GAME_WIDTH/2, GAME_HEIGHT/2 + 60, '¡ TIRAR !', {
    ...
    // Botón cerrar — oculto hasta que haya resultado
  ...
  tirar() {
    ...
    this.ruletas.forEach((ruleta, i) => {
      ...
      this.time.delayedCall(1000 + i * 800, () => {
        ...
        if (i === 2) {
          this.time.delayedCall(400, () => this.mostrarResultado());
        }
      });
    });
  }
